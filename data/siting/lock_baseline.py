...
def main():
    ...
    for lga, q in LGA_QUOTAS.items():
        ...
        for _, r in zero_fac.iterrows():
            ...
            used.add(key)
            if len(chosen_community) >= q["community"]:
                break
        ...
        for _, r in sub.iterrows():
            ...
            used.add(key)
            if len(chosen_school) >= q["school"]:
                break

        for r in chosen_phc:
            selected.append({**r.to_dict(), "channel": "PHC"})
        for r in chosen_community:
            selected.append({**r.to_dict(), "channel": "community"})
        for r in chosen_school:
            selected.append({**r.to_dict(), "channel": "school"})

    alloc = pd.DataFrame(selected)
    channel_order = {"PHC": 0, "school": 1, "community": 2}
    alloc["co"] = alloc.channel.map(channel_order)
    alloc = (
        alloc.sort_values(["co", "lganame", "score"], ascending=[True, True, False])
        .drop(columns=["co"])
    ...
    alloc["stable_core"] = alloc.apply(lambda r: (r.lganame, r.wardname) in stable_core, axis=1)
    ...
    def rationale(r):
        ...
        ch = r["channel"]
        if rk <= 5:
            bits.append("Rank " + str(rk) + " overall")
        else:
            bits.append("Rank " + str(rk))
        if pop >= 5000:
            bits.append(f"{pop:,} children under 5")
        elif pop == 0:
            bits.append("zero pop captured (mangrove)")
        else:
            bits.append(str(pop) + " u5")
        if pd.notna(no2) and no2 >= 25:
            bits.append(f"high NO2 ({no2:.1f} umol/m^2)")
        if flare >= 0.5:
            bits.append(f"heavy flare ({flare:.2f} BCM)")
        elif flare > 0:
            bits.append("flare exposure")
        if phc == 0:
            bits.append("zero PHCs (equity)")
        if ch == "PHC" and phc >= 1:
            bits.append(str(phc) + " PHCs to host")
        if r["stable_core"]:
            bits.append("stable across all weightings")
        return "; ".join(bits)
