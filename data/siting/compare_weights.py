...
def score_under(w, df):
    ...
    out["score"] = (
        ...
        w["coverage_gap"] * (1 - norm01(out["phc_per_1k_u5"].clip(upper=out["phc_per_1k_u5"].quantile(0.95))))
    ).round(3)
    return out
...
def allocate(scored):
    ...
    for lga, quota in LGA_QUOTAS.items():
        ...
        for _, r in phc_candidates.iterrows():
            ...
            chosen_phc.append(r)
            used.add(key)
            if len(chosen_phc) >= quota["phc"]: break
        ...
        for _, r in sub.iterrows():
            ...
            used.add(key)
            if len(chosen_school) >= quota["school"]: break
        ...
        for _, r in zero_fac.iterrows():
            ...
            used.add(key)
            if len(chosen_community) >= quota["community"]: break
        for _, r in sub.iterrows():
            if len(chosen_community) >= quota["community"]: break
            key = (r.lganame, r.wardname)
            if key in used: continue
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"Per-weighting comparison (vs baseline):")
    ...
    for name, alloc in allocations.items():
        for _, r in alloc.iterrows():
            rows.append({"weighting": name, "lganame": r.lganame, "wardname": r.wardname, "channel": r.channel})
