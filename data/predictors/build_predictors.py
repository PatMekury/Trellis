...
def main():
    ...
    flares_clean = flares.rename(columns={
        "n_flare_sites":"flare_n_sites","total_bcm_13yr":"flare_total_bcm_13yr",
        "max_site_bcm":"flare_max_site_bcm","last_year_bcm":"flare_last_year_bcm",
        "operators":"flare_operators",
    })[["lganame","wardname","flare_n_sites","flare_total_bcm_13yr",
         "flare_max_site_bcm","flare_last_year_bcm","flare_operators"]]
    fac_clean = fac.rename(columns={
        "n_facilities":"fac_total","n_phc":"fac_phc","n_secondary":"fac_secondary",
        "n_tertiary":"fac_tertiary","n_public":"fac_public","n_private":"fac_private",
    })[["lganame","wardname","fac_total","fac_phc","fac_secondary","fac_tertiary",
         "fac_public","fac_private"]]
    ...
    static_int = ["flare_n_sites","fac_total","fac_phc","fac_secondary","fac_tertiary",
                  "fac_public","fac_private","pop_total","pop_under5"]
    static_float = ["flare_total_bcm_13yr","flare_max_site_bcm","flare_last_year_bcm"]
    for c in static_int: out[c] = out[c].fillna(0).astype(int)
    for c in static_float: out[c] = out[c].fillna(0.0)
    ...
    cols = [
        "lganame","wardname","ym","year","month",
        "no2_mol_m2","no2_umol_m2","no2_valid_pixels",
        "rainfall_mm","rainfall_valid_pixels",
        "t2m_mean","t2m_max","t2m_min","ts_mean","temp_n_days",
        "flare_n_sites","flare_total_bcm_13yr","flare_max_site_bcm",
        "flare_last_year_bcm","flare_operators",
        "fac_total","fac_phc","fac_secondary","fac_tertiary","fac_public","fac_private",
        "pop_total","pop_under5","fac_per_1k_pop","phc_per_1k_u5",
    ]
    out = out[cols]
    ...
    # Coverage report
    print(f"\nWard-month coverage:")
    print(f"  NO2:                  {out.no2_mol_m2.notna().sum()} ({out.no2_mol_m2.notna().mean()*100:.1f}%)")
    print(f"  rainfall:             {out.rainfall_mm.notna().sum()} ({out.rainfall_mm.notna().mean()*100:.1f}%)")
    ...
    print(f"  flares (>=1):         {(out.flare_n_sites > 0).sum()} ({(out.flare_n_sites>0).mean()*100:.1f}%)")
    print(f"  facilities (>=1):     {(out.fac_total > 0).sum()} ({(out.fac_total>0).mean()*100:.1f}%)")
    print(f"  rainfall lag1m:       {out.rainfall_mm_lag1m.notna().sum()} ({out.rainfall_mm_lag1m.notna().mean()*100:.1f}%)")
    print(f"  rainfall lag3m:       {out.rainfall_mm_lag3m.notna().sum()} ({out.rainfall_mm_lag3m.notna().mean()*100:.1f}%)")
    ...
    merged = pilot.set_index(["lganame","wardname"]).join(
        latest_static.drop(columns=["ym","year","month"], errors="ignore")
    ).reset_index()
