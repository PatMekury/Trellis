    ...
    mvp_payload.append(
        {
            ...
            "forecast_year": 2026,
            "forecast_month": 5,
            "no2_forecast_umol_m2": float(r["no2_umol_m2_forecast"])
            if not pd.isna(r["no2_umol_m2_forecast"])
            else None,
            "rainfall_forecast_mm": float(r["rainfall_mm_forecast"])
            if not pd.isna(r["rainfall_mm_forecast"])
            else None,
            "t2m_forecast_c": float(r["t2m_mean_forecast"]) if not pd.isna(r["t2m_mean_forecast"]) else None,
            "no2_climatology": float(r["no2_umol_m2_climatology"])
            if not pd.isna(r["no2_umol_m2_climatology"])
            else None,
            "tier": r["tier"],
            "model": "xgboost_v1",
...
print(
    forecasts.groupby("lganame")["no2_umol_m2_forecast"]
...
print(
    forecasts.nlargest(10, "no2_umol_m2_forecast")[
        [
            "lganame",
            "wardname",
            "no2_umol_m2_forecast",
            "no2_umol_m2_climatology",
