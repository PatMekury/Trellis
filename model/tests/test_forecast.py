...
def test_predictor_table_has_expected_shape():
    ...
    expected = {"lganame", "wardname", "year", "month",
                "no2_umol_m2", "rainfall_mm", "t2m_mean",
                "pop_under5", "fac_phc", "flare_total_bcm_13yr"}
...
def test_forecast_json_schema():
    p = REPO_ROOT / "mvp" / "forecast_may_2026.json"
    fc = json.loads(p.read_text())
    required = {"lganame", "wardname", "no2_forecast_umol_m2",
                "tier", "forecast_year", "forecast_month"}
    for row in fc:
...
def test_forecast_lga_set_matches_pilot():
    ...
    expected = {"Warri South", "Warri South-West",
