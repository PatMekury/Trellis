            ...
            [
                {
                    "lganame": "Warri South",
                    "wardname": "Ekurede",
                    "no2_forecast_umol_m2": 30.0,
                    "no2_climatology": 23.0,
                    "tier": "likely",
                    "forecast_year": 2026,
                    "forecast_month": 5,
                },
                {
                    "lganame": "Warri South",
                    "wardname": "Ubeji",
                    "no2_forecast_umol_m2": 22.0,
                    "no2_climatology": 21.0,
                    "tier": "possible",
                    "forecast_year": 2026,
                    "forecast_month": 5,
                },
                {
                    "lganame": "Bomadi",
                    "wardname": "Bomadi",
                    "no2_forecast_umol_m2": 14.0,
                    "no2_climatology": 14.5,
                    "tier": "watch",
                    "forecast_year": 2026,
                    "forecast_month": 5,
                },
...
def test_enroll_duplicate_409(client):
    payload = {"phone": "+2348011110001", "lganame": "Warri South", "wardname": "Ekurede", "lang": "en"}
...
def test_dispatch_sends_for_alert_tiers_only_by_default(client):
    ...
    for ward, name in [
        ("Ekurede", "+2348011110001"),
        ("Ubeji", "+2348011110002"),
        ("Bomadi", "+2348011110003"),
    ]:
        lga = "Warri South" if ward in ("Ekurede", "Ubeji") else "Bomadi"
        client.post("/enroll", json={"phone": name, "lganame": lga, "wardname": ward, "lang": "en"})
...
def test_dispatch_idempotent_on_unchanged_tiers(client):
    client.post(
        "/enroll",
        json={"phone": "+2348011110001", "lganame": "Warri South", "wardname": "Ekurede", "lang": "en"},
    )
...
def test_pidgin_template_used_when_lang_pcm(client):
    client.post(
        "/enroll",
        json={"phone": "+2348011110001", "lganame": "Warri South", "wardname": "Ekurede", "lang": "pcm"},
    )
    client.post("/dispatch")
    audit = client.get("/audit").json()
    msg = audit[0]["message"]
